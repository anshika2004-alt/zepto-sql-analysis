...
I. **Database Creation**: The project starts by creating a database named `zepto`.  
   ```sql
    CREATE DATABASE zepto_sql_project
   ```
   
II. **Table Creation**: A table named `zepto` is created to store the inventory data. The table structure includes columns for skuid, category, name, mrp, discountPercent, availableQuantity, discountedSellingPrice, weightInGms, outOfStock, quantity
   ```sql
   CREATE TABLE zepto
   (
...
I. **Record Count**: Determine the total number of records in the dataset.  
...
II. **Null Value Check**: Check for any null values in the dataset and delete records with missing data.
   ```sql
   SELECT *
   FROM zepto
   WHERE name IS NULL
   ...
   quantity IS NULL;
   ```
III. **Category Count**: Identify all unique product categories in the dataset.
...
IV. **Product In stock Vs Out Of Stock**: Check if the products are in stock or are out of stock.
  ```sql
...
V. **Product Names Present Multiple Times**: Lists products that appear in the data more than once.
  ```sql
...
VII. **Delete Products Where Price = 0**
```sql
DELETE FROM zepto
WHERE mrp = 0;
```
VIII. **Convert Paise To Rupees**
```sql
UPDATE zepto
SET mrp = mrp /100.0,
...
9. **What is the total inventory weight per category**
     ```sql
     SELECT category,
